<?php
/*
  Plugin Name: Jobs Attributes Plugin
  Plugin URI: https://osclasspoint.com/
  Description: Plugin extends a category of items to store jobs attributes such as salary, requirements, timetable etc
  Version: 3.4.0
  Author: MB Themes
  Author URI: https://osclasspoint.com
  Author Email: info@osclasspoint.com
  Short Name: jobs_attributes
  Plugin update URI: jobs_attributes
  Support URI: https://forums.osclasspoint.com/free-plugins/
  Product Key: xJTuuOyytVDITjNjMlSC
*/
...
function job_call_after_install() {
  ...
  osc_set_preference('cv_email', '', 'jobs_attributes', 'STRING');
  osc_set_preference('allow_cv_upload', '0', 'jobs_attributes', 'BOOLEAN');
  osc_set_preference('allow_cv_unreg', '1', 'jobs_attributes', 'BOOLEAN');
  osc_set_preference('send_me_cv', '0', 'jobs_attributes', 'BOOLEAN');
  
  osc_set_preference('version', 320, 'jobs_attributes', 'INTEGER');
}
...
function job_call_after_uninstall() {
  ...
  osc_delete_preference('cv_email', 'jobs_attributes');
  osc_delete_preference('allow_cv_upload', 'jobs_attributes');
  osc_delete_preference('allow_cv_unreg', 'jobs_attributes');
  osc_delete_preference('send_me_cv', 'jobs_attributes');

  osc_delete_preference('version', 'jobs_attributes');
}
...
function job_check_update()
{
  ...
  // UPDATE PROCESS
  if(osc_get_preference('version','jobs_attributes')<310 && (osc_get_preference('allow_cv_unreg', 'jobs_attributes')==1 || osc_get_preference('allow_cv_unreg', 'jobs_attributes')==0)) {
    osc_delete_preference('salary_min', 'jobs_attributes');
    osc_delete_preference('salary_max', 'jobs_attributes');
    osc_delete_preference('salary_step', 'jobs_attributes');
    osc_set_preference('version', 310, 'jobs_attributes');
    
    ModelJobs::newInstance()->upgradeTo310();
    
  }

  osc_set_preference('version', 320, 'jobs_attributes');
