...
class UsersDataTable extends DataTable {
  ...
  private function processData($users) {
    if(!empty($users)) {
      ...
      foreach($users as $aRow) {
        ...
        $options[]  = '<a href="' . osc_user_public_profile_url($aRow['pk_i_id'], $aRow) . '" target="_blank">' . __('Public profile') . '</a>';
        ...
        $row['phone'] = trim((string)($aRow['s_phone_mobile'] <> '' ? $aRow['s_phone_mobile'] : $aRow['s_phone_land']));
  ...
  private function getDBParams($_get) {
    ...
    if(@$_get['userId'] != '') {
      if(substr($_get['userId'], 0, 1) == '"' || substr($_get['userId'], -1) == '"') {
    ...
    if(@$_get['s_email'] != '') {
      if(substr($_get['s_email'], 0, 1) == '"' || substr($_get['s_email'], -1) == '"') {
        $esc_email = User::newInstance()->dao->escapeStr(str_replace('*','%', str_replace('"','', $_get['s_email'])));
    ...
    if(@$_get['s_name'] != '') {
      if(substr($_get['s_name'], 0, 1) == '"' || substr($_get['s_name'], -1) == '"') {
        $esc_email = User::newInstance()->dao->escapeStr(str_replace('*','%', str_replace('"','', $_get['s_name'])));
      ...
      if(@$_get['userId'] == '') {
        if(substr($_get['user'], 0, 1) == '"' || substr($_get['user'], -1) == '"') {
          $esc_user = User::newInstance()->dao->escapeStr(str_replace('*','%', str_replace('"','', $_get['user'])));
          $this->conditions["(s_email = '" . $esc_user . "' OR s_name = '" . $esc_user . "' OR s_username = '" . $esc_user . "'  OR s_website = '" . $esc_user . "' OR s_phone_mobile = '" . $esc_user . "' OR s_phone_land = '" . $esc_user . "')"] = null;
        } else {
          $esc_user = User::newInstance()->dao->escapeStr(str_replace('%%', '%', str_replace('*','%', '%' . $_get['user'] . '%')));
          $this->conditions["(s_email LIKE '" . $esc_user . "' OR s_name LIKE '" . $esc_user . "' OR s_username LIKE '" . $esc_user . "' OR s_website LIKE '" . $esc_user . "' OR s_phone_mobile LIKE '" . $esc_user . "' OR s_phone_land LIKE '" . $esc_user . "')"] = null;
        }
      } else {
    ...
    if(@$_get['s_username']!='') {
      if(substr($_get['s_username'], 0, 1) == '"' || substr($_get['s_username'], -1) == '"') {
        $esc_username = User::newInstance()->dao->escapeStr(str_replace('*','%', str_replace('"','', $_get['s_username'])));
    ...
    if(@$_get['s_phone']!='') {
      if(substr($_get['s_phone'], 0, 1) == '"' || substr($_get['s_phone'], -1) == '"') {
        $esc_phone = User::newInstance()->dao->escapeStr(str_replace('*','%', str_replace('"','', $_get['s_phone'])));
    ...
    if(@$_get['s_zip']!='') {
      if(substr($_get['s_zip'], 0, 1) == '"' || substr($_get['s_zip'], -1) == '"') {
        $esc_zip = User::newInstance()->dao->escapeStr(str_replace('*','%', str_replace('"','', $_get['s_zip'])));
        $this->conditions["s_zip = '" . $esc_zip . "'"] = null;
      } else {
        $esc_zip = User::newInstance()->dao->escapeStr(str_replace('%%', '%', str_replace('*','%', '%' . $_get['s_zip'] . '%')));
