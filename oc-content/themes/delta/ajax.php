...
if(@$_GET['ajaxLoc2'] == '1') {
  ...
  echo del_locbox_short(Params::getParam('country'), Params::getParam('region'), Params::getParam('city'), $level);
  exit;
}
...
if(@$_GET['ajaxQuery'] == '1' && isset($_GET['pattern'])) {
  ...
  $pattern = trim(osc_esc_js(Params::getParam('pattern')));
...
if(@$_GET['ajaxItem'] == '1' && @$_GET['sPattern'] <> '') {
  $pattern = trim(osc_esc_js(Params::getParam('sPattern')));
  $max = 12;
  ...
  foreach( $prepare as $i => $p ) {
    ...
    $prepare[$i]['i_price'] = del_ajax_item_format_price($prepare[$i]['i_price'], $prepare[$i]['fk_c_currency_code']);
...
if(@$_GET['ajaxPhoneClick'] == '1' && @$_GET['itemId'] > 0) {
  del_increase_clicks(Params::getParam('itemId'), Params::getParam('itemUserId'));
  echo 1;
  exit;
}
