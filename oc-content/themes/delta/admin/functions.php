...
// ADMIN MENU
function del_menu($title = NULL) {
  echo '<link href="' . osc_base_url() . 'oc-content/themes/delta/admin/css/admin.css?v=' . date('YmdHis') . '" rel="stylesheet" type="text/css" />';
  echo '<link href="' . osc_base_url() . 'oc-content/themes/delta/admin/css/bootstrap-switch.css" rel="stylesheet" type="text/css" />';
  echo '<link href="' . osc_base_url() . 'oc-content/themes/delta/admin/css/tipped.css" rel="stylesheet" type="text/css" />';
  ...
  echo '<script src="' . osc_base_url() . 'oc-content/themes/delta/admin/js/admin.js?v=' . date('YmdHis') . '"></script>';
  echo '<script src="' . osc_base_url() . 'oc-content/themes/delta/admin/js/tipped.js"></script>';
  echo '<script src="' . osc_base_url() . 'oc-content/themes/delta/admin/js/bootstrap-switch.js"></script>';


  $current = basename(Params::getParam('file'));

  $links = array();
  $links[] = array('file' => 'configure.php', 'icon' => 'fa-wrench', 'title' => __('Configure', 'delta'));
  $links[] = array('file' => 'banner.php', 'icon' => 'fa-clone', 'title' => __('Advertisement', 'delta'));
  $links[] = array('file' => 'category.php', 'icon' => 'fa-cogs', 'title' => __('Category Icons', 'delta'));
  $links[] = array('file' => 'header.php', 'icon' => 'fa-desktop', 'title' => __('Header Logo', 'delta'));
  $links[] = array('file' => 'plugins.php', 'icon' => 'fa-puzzle-piece', 'title' => __('Plugins', 'delta'));

  if( $title == '') { $title = __('Configure', 'delta'); }

  $text  = '<div class="mb-head">';
  $text .= '<div class="mb-head-left">';
  $text .= '<h1>' . $title . '</h1>';
  $text .= '<h2>Delta Osclass Theme</h2>';
  $text .= '</div>';
  $text .= '<div class="mb-head-right">';
  $text .= '<ul class="mb-menu">';

  foreach($links as $l) {
    $text .= '<li><a href="' . osc_admin_base_url(true) . '?page=appearance&action=render&file=oc-content/themes/delta/admin/' . $l['file'] . '" class="' . ($l['file'] == $current ? 'active' : '') . '"><i class="fa ' . $l['icon'] . '"></i><span>' . $l['title'] . '</span></a></li>';
  }
...
// List of categories
function del_has_subcategories_special($categories, $deep = 0) {
  ...
  foreach($categories as $c) {
    $c_extra = del_category_extra($c['pk_i_id']);
    ...
    echo '<div class="mb-col-1_1_2"><a class="add_img" id="small' . $c['pk_i_id'] . '" href="#">' . __('Add small image', 'delta') . '</a></div>';
    ...
    echo '<div class="mb-col-1_1_2"><a class="add_img" id="large' . $c['pk_i_id'] . '" href="#">' . __('Add large image', 'delta') . '</a></div>';
    echo '<div class="mb-col-1_1_2 mb-align-left fa-icon"><a class="add_fa" id="fa-icon' . $c['pk_i_id'] . '" href="#" title="To remove icon click on link and leave input empty.">' . __('Add / remove icon', 'delta') . '</a>';
    ...
    if(isset($_POST['color' . $c['pk_i_id']]) and $_POST['color' . $c['pk_i_id']] == '') { 
      echo  __('Add / remove color', 'delta');
      ...
      echo __('Color', 'delta') . ': ' . $color . '<span class="show-color" style="background:' . $color . '"></span>';
    }
    ...
    if(isset($c['categories']) && is_array($c['categories']) && !empty($c['categories'])) {
      del_has_subcategories_special($c['categories'], $deep+1);
    }   
